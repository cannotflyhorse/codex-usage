...
实时统计每次对话消耗了多少 token、缓存命中了多少、折合多少钱。数据全部来自本机 Codex 日志，不联网、不上传。
...
窗口很小（约 232×145 逻辑像素），只显示四行：本次对话的花费与 token、缓存命中数与命中率、最近 12 次调用的迷你柱状图、今日/本月金额与当前峰谷时段。鼠标悬停在窗口上会弹出更详细的提示（项目名、调用次数、输入输出、缓存省下的钱、累计花费）。
